...
var default_alipay_config = {
    ...
    seller_id: '2088911869365628',//卖家支付宝用户号 必填，可能和合作身份者id相同
    host: '', //域名
...
Alipay.prototype.create_direct_pay_by_user_phone = function (data, res) {
    assert.ok(data.out_trade_no && data.subject && data.total_fee);
    console.log("---------------create_direct_pay_by_user_phone---------------");
    //建立请求
    var alipaySubmit = new AlipaySubmit(this.alipay_config);
 ...
 * @param res
 */
Alipay.prototype.create_direct_pay_by_user_shamophone = function (data, res, notify_url, sync_url) {
    assert.ok(data.out_trade_no && data.subject && data.total_fee);
    console.log("--------------沙漠支付---------------");
    //建立请求
    var alipaySubmit = new AlipaySubmit(this.alipay_config);
    ...
    var parameter = {
        ...
        , payment_type: '1' //支付类型
        , app_pay: 'Y' //是否使用支付宝客户端支付
        , notify_url: notify_url//服务器异步通知页面路径,必填，不能修改, 需http://格式的完整路径，不能加?id=123这类自定义参数
        , return_url: sync_url//页面跳转同步通知页面路径 需http://格式的完整路径，不能加?id=123这类自定义参数，不能写成http://localhost/
        , seller_id: this.alipay_config.seller_id  //卖家支付宝用户号 必填
        , _input_charset: this.alipay_config['input_charset'].toLowerCase().trim()
    };
    _.merge(parameter, data);
    ...
    try {
        ...
        alipayNotify.verifyNotify(_POST, function (verify_result) {
            ...
            if (verify_result) {//验证成功
                ...
                var trade_status = _POST['trade_status'];
                //其他
                var notify_type = _POST['notify_type'];
                ...
                if (trade_status == 'TRADE_FINISHED' || trade_status == 'TRADE_SUCCESS') {
                    console.log("异步通知验证成功");
                    connection.query("SELECT COUNT(*) AS num FROM w_game_recharge_log WHERE out_trade_no = ? ", [out_trade_no], function (err, result) {
                        if (err) {
                            console.log('查询失败 - ', err.message);
                            res.send("fail,--支付失败！" + err.message);
                            return;
                        }
                        if (result[0].num > 0) {
                            console.log("订单已经存在，请联系管理员！");
                            res.send("订单已经存在，请联系支付宝客户！");
                            return;
                        } else {
                            //写数据
                            var addSql = "INSERT INTO w_game_recharge_log(out_trade_no,trade_no,total_fee,`subject`,body,pay_time,state,pay_way) VALUES(?,?,?,?,?,?,?,?);";
                            var addSqlParams = [out_trade_no, trade_no, total_fee * 100, subject, body, new Date(), 2, 1];
                            connection.query(addSql, addSqlParams, function (err, result) {
                                if (err) {
                                    console.log('插入错误 ', err.message);
                                    return;
                                }
                                console.log("支付记录数据插入成功");
                                //通知游戏上充值(????应该通知多次)
                                tradeNo.notifyGameCompanyToRechargr(out_trade_no);
...
Alipay.prototype.create_direct_pay_by_user_return = function (req, res) {
    ...
    alipayNotify.verifyReturn(_GET, function (verify_result) {
        if (verify_result) {//验证成功
            ...
            //交易状态
            var trade_status = _GET['trade_status'];
            var notify_type = _GET['notify_type'];
            var service = _GET['service'];
            var subject = _GET['subject'];
            var total_fee = _GET['total_fee'];
            var body = _GET['body'];
            console.log("--------同步通知begin--------");
            console.log("out_trade_no:" + out_trade_no);
            console.log("trade_no:" + trade_no);
            console.log("trade_status:" + trade_status);
            ...
            /*connection.query("SELECT COUNT(*) AS num FROM w_game_recharge_log WHERE out_trade_no = ? ",[out_trade_no],function (err, result) {
                ...
                if(result[0].num>0){
                    console.log("订单已经存在，请联系支付宝客户");
                    res.send("订单已经存在，请联系支付宝客户！");
                    return;
                }else{
                    //写数据
                    var addSql = "INSERT INTO w_game_recharge_log VALUES(out_trade_no,trade_no,total_fee,`subject`,body,pay_time,state,pay_way)";
                    var addSqlParams = [out_trade_no,trade_no,total_fee,subject,body,new Date(),2,1];
                    connection.query(addSql,addSqlParams,function (err, result) {
                        if(err){
            ...
            // }
            console.log("return url IS OK");
            console.log("**************支付同步通知接口end******************");
            //res.send("success,hahhahahha----支付成功！");
            res.redirect(302, "http://wap.pdl.jsy86.com/paySuccess.html");
        }
        else {
            //验证失败
            self.emit("verify_fail");
